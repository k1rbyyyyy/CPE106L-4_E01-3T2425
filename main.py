...
# -------------------- ✅ My Requests --------------------
@app.get("/my_requests")
def get_my_requests(user_id: int = Query(..., description="User ID making the request")):
    try:
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        cursor.execute("""
        ...
        return {"my_requests": results}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

# -------------------- ✅ View Stats --------------------
@app.get("/view_stats")
def view_stats(user_id: int = Query(...)):
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    ...
    # Count user requests
    cursor.execute("SELECT COUNT(*) FROM match_requests WHERE requester_id = ?", (user_id,))
    request_count = cursor.fetchone()[0]

    # Count user matches (can be in either column)
    cursor.execute("""
